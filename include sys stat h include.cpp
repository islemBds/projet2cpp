...
class Rcmd {
    public:
        ...
        int idfct;
        int idDestinataire;
        Rcmd(int x, string s, int f, int d) {id = x; type = s; idfct = f; idDestinataire = d;}
...
class fct : public Prsn {
    public:
        fct(int x, string n, int z) {id = x; nom = n; zoneGeo = z;}
        
};

class Habitant : public Prsn {
    public:
...
int main() {
    ...
    while (1=1) { // zid wa7da ghira jabli el pw

        cout << "a3tini asme database yr7am babaQ :" << endl;
        ...
        if (fileExists(filename)) { 
            ...
            cout << "Database hadi kayna ,ara tchu  password:" << endl;
            readString(password);
            clearScreen();

          // ro7 le str li mkhabi fih  el paswrd
            ifstream fileIn;
    ...
    while (true) { // While loop for looping program and not closing when done
        ...
        if (choice == 0) { // -------------- ADDING A NEW LINE -------------- //
            ...
            cout << "wsh tzid? (0:fct, 1:Habitant, 2:Rcmd):" << endl;
            cin >> choice;
            ...
            while (getline(fileIn, line)) {
                ...
                if (classType == 0) {
                    ...
                    fct fac(stoi(vector_input[1]),vector_input[2],stoi(vector_input[3]));
                    ...
                    Rcmd reco(stoi(vector_input[1]),vector_input[2],stoi(vector_input[3]),stoi(vector_input[4]));
...
string makeNewData(int &choice) {
    ...
    if (choice == 0 || choice == 1) {
        ...
        if (choice == 0) {
            Prsn = fct(stoi(vector_input[0]),vector_input[1],stoi(vector_input[2]));
        ...
        Rcmd reco(stoi(vector_input[0]),type[choice],stoi(vector_input[1]),stoi(vector_input[2]));
